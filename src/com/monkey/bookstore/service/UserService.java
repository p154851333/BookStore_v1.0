...
import com.monkey.bookstore.dao.UserDao;
...
public class UserService {
	...
	* @date		2017年3月25日 下午9:26:14
	...
	public void regist(User form) throws UserException{
		...
		User user = dao.findUserByName(form.getUsername());
		if (user!=null) {
			throw new UserException("*此用户名已被注册！");
		}
		user = dao.findUseByEmail(form.getEmail());
		if (user!=null) {
	...
	* @date		2017年3月26日 下午4:41:58
	...
	public void active(String code) throws UserException{
		...
		User user = dao.findUseByCode(code);
		if (user==null) {
			//数据库中没有对应的激活码账户
	...
	public User login(User formUser) throws UserException{
		//通过用户名查询dao
		User user = dao.findUserByName(formUser.getUsername());
		if (user==null) {
			//查询为空。说明么有用户
